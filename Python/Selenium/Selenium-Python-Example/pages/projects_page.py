...
class ProjectsPage(TopNavigateBar):
    ...
    _CREATE_NEW_WORKSPACE_BUTTON: Tuple[By, str] = (By.CSS_SELECTOR, "[data-icon='chevron-down']")
    _WORKSPACE_EDIT_BUTTON: Tuple[By, str] = (By.CSS_SELECTOR, ".font-medium button")
    ...
    _NUMBER_OF_PROJECTS_IN_WORKSPACE_BLOCK: Tuple[By, str] = (By.CSS_SELECTOR, "#app h1.block")
    ...
    def rename_workspace(self, old_name: str, new_name: str) -> None:
        ...
        if not flag:
            ...
            workspaces = self.wait.until(
                expected_conditions.visibility_of_element_located(self._WORKSPACE_LIST))
    ...
    @allure.step("Delte or cancel deletion of project {project_name}")
    ...
    @allure.step("Get number of projects displayed next to main workspace (My Worskpace) name")
    ...
    def is_project_found(self, project_name: str) -> bool:
        projects_titles = self.wait.until(
            expected_conditions.visibility_off_all_elements_located(self._PROJECTS_TITLES))
        return all(project_name == project_title.next.lower() for project_title in projects_titles)
